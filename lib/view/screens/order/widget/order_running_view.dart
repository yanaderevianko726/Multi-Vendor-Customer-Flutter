...
class OrderRunningView extends StatefulWidget {
  const OrderRunningView({
    Key key,
  }) : super(key: key);
  @override
  State<OrderRunningView> createState() => _OrderRunningViewState();
}

class _OrderRunningViewState extends State<OrderRunningView> {
  @override
  void initState() {
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<OrderController>(
      builder: (orderController) {
        ...
        int pageSize = 1;
        int offset = 1;
        if (orderController.deliveryRunningOrderList != null) {
          orderList = orderController.deliveryRunningOrderList;
          if (orderController.runningPageSize != null) {
            pageSize = (orderController.runningPageSize / 10).ceil();
          }
          if (orderController.deliveryOrdersOffset != null) {
            offset = orderController.deliveryOrdersOffset;
          }
        }
        scrollController.addListener(() {
          if (scrollController.position.pixels ==
              ...
              !paginate) {
            if (offset < pageSize) {
              Get.find<OrderController>().setOffset(offset + 1, true);
              print('end of the page');
              Get.find<OrderController>().showBottomLoader(true);
              Get.find<OrderController>().getDeliveryRunningOrders(offset + 1);
            }
          }
        });
        ...
        return orderList != null
            ? orderList.length > 0
                ? RefreshIndicator(
                    onRefresh: () async {
                      await orderController.getDeliveryRunningOrders(1);
                    },
                    child: Container(
