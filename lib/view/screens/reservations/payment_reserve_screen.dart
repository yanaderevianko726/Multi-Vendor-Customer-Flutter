...
class PaymentReserveScreen extends StatefulWidget {
  final OrderModel orderModel;
  PaymentReserveScreen({
    @required this.orderModel,
  });
  @override
  _PaymentReserveScreenState createState() => _PaymentReserveScreenState();
}

class _PaymentReserveScreenState extends State<PaymentReserveScreen> {
  String selectedUrl;
  double value = 0.0;
...
class MyInAppReserveBrowser extends InAppBrowser {
  ...
  void _redirect(String url) {
    if (_canRedirect) {
      ...
      if (_isSuccess) {
        Get.find<ReservationController>().setPaymentStatus('paid');
        Get.find<ReservationController>()
            .updateReservationWithPay()
            .then((value) async {
