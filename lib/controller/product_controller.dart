...
class ProductController extends GetxController implements GetxService {
  ...
  List<Product> _celebProductList = [];
  ...
  List<Product> get celebProductList => _celebProductList;
  bool get isLoading => _isLoading;
  List<int> get variationIndex => _variationIndex;
  int get quantity => _quantity;
  List<bool> get addOnActiveList => _addOnActiveList;
  List<int> get addOnQtyList => _addOnQtyList;
  String get popularType => _productType;
  ...
  Future<void> getPopularProductList(
    ...
    Response response =
        await productRepo.getPopularProductList(type, limit, offset);
  ...
  Future<void> getCelebProductList(
    bool reload,
    bool notify, {
    int limit = 10,
    int offset = 1,
    ...
    Response response =
        await productRepo.getPopularProductList('all', limit, offset);
    if (response.statusCode == 200) {
      List<Product> __product = [];
      __product.addAll(ProductModel.fromJson(response.body).products);
      print('=== pLength: ${__product.length}');

      _celebProductList = [];
      var index = 0;
      __product.forEach((element) {
        Product product = __product[index].copyWith();
        _celebProductList.add((product));
        index++;
      });
      _isLoading = false;
    } else {
  ...
  Future<void> getReviewedProductList(
    ...
    Response response =
        await productRepo.getReviewedProductList(type, limit, offset);
    if (response.statusCode == 200) {
