...
class AuthController extends GetxController implements GetxService {
  ...
  void setDeviceId(String __deviceId) {
  ...
  Future<ResponseModel> registerGuest(SignUpBody signUpBody) async {
    _isLoading = true;
    update();
    Response response = await authRepo.registerGuest(signUpBody);
    ResponseModel responseModel;
    if (response.statusCode == 200) {
      authRepo.saveUserToken(response.body['token']);
      await authRepo.updateToken();
      if (response.body['user'] != null) {
        Map<String, dynamic> userMap = response.body['user'];
        Get.find<UserController>().setUserInfoWithMap(
          userMap,
          signUpBody.lName,
        );
  ...
  Future<ResponseModel> loginGuest(
    String email,
  ) async {
    _isLoading = true;
    update();
    Response response = await authRepo.loginGuest(
      email: email,
      password: _deviceId,
    );
    ResponseModel responseModel;
    if (response.statusCode == 200) {
      ...
      if (response.body['user'] != null) {
        Map<String, dynamic> userMap = response.body['user'];
        Get.find<UserController>().setUserInfoWithMap(userMap, deviceId);
        saveUserEmailAndPassword(
          '$email',
          '$_deviceId',
        );
